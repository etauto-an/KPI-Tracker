...
def add_daily_metrics(rep_id):
    try:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (rep_id, date, scheduled_calls, live_calls, offers, closed, cash_collected, contract_value))
        conn.commit()

        print(f"Metrics for sales rep ID {rep_id} added successfully.")
    except ValueError:
        print("Invalid input. Please enter valid numbers for calls, offers, deals, cash, and contract values.")
...
def calculate_kpis(rep_id, name):
    ...
    total_scheduled_calls, total_live_calls, total_offers, total_closed, total_cash_collected, total_contract_value = data
    ...
    # KPI calculations
    show_percentage = (total_live_calls / total_scheduled_calls) * 100 if total_scheduled_calls > 0 else 0
    offer_percentage = (total_offers / total_live_calls) * 100 if total_live_calls > 0 else 0
    close_percentage = (total_closed / total_offers) * 100 if total_offers > 0 else 0
    cash_per_call = total_cash_collected / total_live_calls if total_live_calls > 0 else 0
    revenue_per_call = total_contract_value / total_live_calls if total_live_calls > 0 else 0
